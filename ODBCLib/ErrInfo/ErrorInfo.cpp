...
ODBCLib::CErrorInfo::CErrorInfo(SQLSMALLINT handleType, SQLHANDLE sqlHandle):
	m_type(handleType), m_handle(sqlHandle), m_errors() {

	SQLSMALLINT record = 1;
	wchar_t sqlState[6 + 1] = {L""};
	...
	while(record <= count) {
		...
		if(SQL_SUCCEEDED(ret)) {
			...
			if(	(ret == SQL_SUCCESS) ||
				...
				// その他のエラー
				std::wcerr << L"CErrorInfo::CErrorInfo() SQLGetDiagRecW" <<
					L" 成功したが取得メッセージ長が異常" <<
					L" record=" << record <<
					L" len=" << errorMsgLen <<
					std::endl;
...
/*
	bool bLoop = true;
	while(bLoop) {
		SQLRETURN ret = ::SQLGetDiagRecW(m_type, m_handle, record, sqlState, &nativeError, message, messageLen, &errorMsgLen);
		if(SQL_SUCCEEDED(ret)) {
			// 成功か、追加情報ありだけどメッセージ長が足りている場合、レコード情報へ追加
			if(	(ret == SQL_SUCCESS) &&
				(	(ret == SQL_SUCCESS_WITH_INFO) &&
					((errorMsgLen > 0) && (errorMsgLen <= messageLen))	)	) {
				m_errors.push_back(Record(sqlState, nativeError, message));
				record++;
			} else if (errorMsgLen > 0) {
				delete [] message;
				messageLen = errorMsgLen;
...
ODBCLib::CErrorInfo::~CErrorInfo() {
}

std::wstring ODBCLib::CErrorInfo::description() const {
	...
	for(unsigned int i = 0; i < GetCount(); ++i) {
		...
		const ODBCLib::CErrorInfo::FieldInfo& fieldInfo = GetFieldInfo(i);
...
SQLINTEGER ODBCLib::CErrorInfo::GetRecordCount() const {
	SQLINTEGER count = 0;
	SQLSMALLINT len = 0;
	SQLRETURN ret = ::SQLGetDiagFieldW(m_type, m_handle, 0, SQL_DIAG_NUMBER, &count, sizeof(count), &len);
	if(ret != SQL_SUCCESS) {
		std::wcerr << L"CErrorInfo::GetRecordCount() SQLGetDiagFieldW error(" << ret << L")" << std::endl;
	}
	return count;
}

ODBCLib::CErrorInfo::FieldInfo ODBCLib::CErrorInfo::GetDiagFieldInfo(SQLSMALLINT record) const {
	SQLRETURN ret = SQL_SUCCESS;
	ODBCLib::CErrorInfo::FieldInfo field;

	// rowNumber
	ret = ::SQLGetDiagFieldW(m_type, m_handle, record, SQL_DIAG_ROW_NUMBER, &field.rowNumber, SQL_IS_INTEGER, NULL);
	if(ret != SQL_SUCCESS) {
		std::wcerr << L"CErrorInfo::GetFieldInfo() SQL_DIAG_ROW_NUMBER SQLGetDiagFieldW error(" << ret << L")" << std::endl;
	}
	// line
	ret = ::SQLGetDiagFieldW(m_type, m_handle, record, SQL_DIAG_SS_LINE, &field.line, SQL_IS_USMALLINT, NULL);
	if(ret != SQL_SUCCESS) {
		std::wcerr << L"CErrorInfo::GetFieldInfo() SQL_DIAG_SS_LINE SQLGetDiagFieldW error(" << ret << L")" << std::endl;
	}
	// msgState
	ret = ::SQLGetDiagFieldW(m_type, m_handle, record, SQL_DIAG_SS_MSGSTATE, &field.msgState, SQL_IS_INTEGER, NULL);
	if(ret != SQL_SUCCESS) {
		std::wcerr << L"CErrorInfo::GetFieldInfo() SQL_DIAG_SS_MSGSTATE SQLGetDiagFieldW error(" << ret << L")" << std::endl;
	}
	// severity
	ret = ::SQLGetDiagFieldW(m_type, m_handle, record, SQL_DIAG_SS_SEVERITY, &field.severity, SQL_IS_INTEGER, NULL);
	if(ret != SQL_SUCCESS) {
		std::wcerr << L"CErrorInfo::GetFieldInfo() SQL_DIAG_SS_SEVERITY SQLGetDiagFieldW error(" << ret << L")" << std::endl;
	}
	// procName
...
std::wstring ODBCLib::CErrorInfo::GetDiagFieldInfo_String(SQLSMALLINT record, SQLSMALLINT diagId) const {
...
std::wcerr << L"CErrorInfo::GetDiagFieldInfo_String() SQLGetDiagFieldW(ID:" << diagId << ") error(" << ret << L")" << std::endl;
			}
		}
	} else {
		// getLength error
std::wcerr << L"CErrorInfo::GetDiagFieldInfo_String() SQLGetDiagFieldW(" << diagId << ") getLength error(" << ret << L")" << std::endl;
	}
